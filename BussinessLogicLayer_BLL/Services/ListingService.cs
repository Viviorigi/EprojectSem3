...
namespace BussinessLogicLayer_BLL.Services
{
	public class ListingService : IListingRepository
	{
		...
		{
			return await _context.Listings.Include(x => x.User).Include(x => x.Category).Include(x => x.City).Include(x => x.Ratings).ThenInclude(r => r.User).Where(x => x.Status == 1 && x.Priority == 1 && x.Category.Status == 1).Take(5).ToListAsync();
		}
